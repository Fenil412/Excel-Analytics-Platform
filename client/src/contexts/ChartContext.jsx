...
export const ChartProvider = ({ children }) => {
  ...
  const fetchHeaders = async (fileId) => {
    ...
    try {
      ...
      try {
        ...
        console.log("Charts headers failed, trying file data endpoint:", chartsError.message)
      ...
      if (response.data) {
        ...
        } else if (response.data.success && response.data.data && response.data.data.headers) {
        ...
        if (Array.isArray(headersData)) {
          headersData = headersData.map((header, index) => {
            if (typeof header === "string") {
              ...
              return {
                name: header.name || header.column || header.field || header.key || `Column ${index + 1}`,
                index: header.index !== undefined ? header.index : index,
                type: header.type || header.dataType || header.columnType || "text",
              }
            }
            return {
              name: `Column ${index + 1}`,
      ...
      const errorMessage = err.response?.data?.error || err.message || "Failed to fetch headers"
  ...
  const generateChart = async (fileId, chartConfig) => {
    ...
    try {
      console.log("Generating chart for fileId:", fileId, "with config:", chartConfig)

      const response = await axios.post(`/api/charts/chart-data/${fileId}`, chartConfig)
  ...
  const generateAggregateData = async (fileId, aggregateConfig) => {
    ...
    try {
      console.log("Generating aggregate data for fileId:", fileId, "with config:", aggregateConfig)

      const response = await axios.post(`/api/charts/chart-aggregate/${fileId}`, aggregateConfig)
      ...
      if (!response.data.success) {
        const errorMsg = response.data.error || response.data.message || "Server returned unsuccessful response"
  ...
  const generate3DChart = async (fileId, chartConfig) => {
    ...
    try {
      const response = await axios.post(`/api/charts/chart-3d/${fileId}`, chartConfig)
