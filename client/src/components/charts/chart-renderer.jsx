  ...
  const processedData = useMemo(() => {
    ...
    if (chartData.data.labels && chartData.data.datasets) {
      ...
      return chartData.data.map((item, index) => ({
        name:
          getDatasetName(item.label || item.name) ||
          item.label ||
          item.name ||
          `Item ${index + 1}`,
            ...
            >
              ...
              <Tooltip
                ...
                formatter={(value, name, props) => [
                  value,
                  getDatasetName(props.payload?.name) || name,
                ]}
              />
              <Legend />
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                dataKey="value"
                label={({ name, percent }) =>
                  `${getDatasetName(name)}: ${(percent * 100).toFixed(1)}%`
                }
              >
    ...
    <div className={`w-full h-full ${className}`}>
      ...
      {chartData && (
        <div className="mt-4 text-sm text-gray-600 dark:text-gray-400 text-center">
          ...
          {datasets.length > 0 && (
            <> • {datasets.length} user datasets available</>
          )}
        </div>
      )}
    </div>
