...
const ChartConfigModal = ({
  ...
  const getDatasetName = (datasetId) => {
    const dataset = datasets.find(
      (d) => d.id === datasetId || d._id === datasetId
    );
    return (
      dataset?.name || dataset?.originalName || datasetId || "Unknown Dataset"
    );
  };

  const validateForm = () => {
    const newErrors = {};
    const is3DChart = ["scatter3d", "surface3d", "mesh3d"].includes(
      config.chartType
    );
                ...
                {datasets.slice(0, 6).map((dataset, index) => (
                  ...
                  >
                    <div className="font-medium truncate">
                      {getDatasetName(dataset.id || dataset._id)}
                    </div>
                    {dataset.isSelected && (
          ...
          {!["histogram", "boxplot"].includes(config.chartType) && (
            <div
              className={`grid grid-cols-1 ${
                is3DChart ? "md:grid-cols-3" : "md:grid-cols-2"
              } gap-4`}
            >
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          ...
          {is3DChart && (
            <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
              ...
              <p className="text-sm text-purple-600 dark:text-purple-400">
                3D charts require three numeric columns for X, Y, and Z axes.
                Make sure to select different columns for each axis to create
                meaningful 3D visualizations.
              </p>
            </div>
          )}
