...
import { Settings, RefreshCw, Download, AlertTriangle, BarChart3 } from "lucide-react"
...
const DataDistributionChart = ({ selectedFileId, className = "" }) => {
  ...
  useEffect(() => {
    if (selectedFileId) {
      console.log("DataDistributionChart: Fetching headers for fileId:", selectedFileId)
      ...
      fetchHeaders(selectedFileId)
        .then((result) => {
          ...
          if (result && result.length > 0) {
            ...
            // Find any column for X-axis (prefer non-numeric for grouping)
            ...
            if (numericHeaders.length > 0) {
              ...
              const xAxisColumn = textHeaders.length > 0 ? textHeaders[0]?.name : result[0]?.name

              // Only auto-generate if we have both axes
              if (yAxisColumn && xAxisColumn && yAxisColumn !== xAxisColumn) {
                const defaultConfig = {
                  chartType: "bar",
                  xAxis: xAxisColumn,
                  yAxis: yAxisColumn,
                  ...
                  limit: 100,
                  chartTitle: `${yAxisColumn} by ${xAxisColumn}`,
                }
                console.log("Auto-generating chart with config:", defaultConfig)
                handleGenerateChart(defaultConfig)
              } else if (numericHeaders.length > 0) {
                // Fallback to histogram for single numeric column
                const defaultConfig = {
                  chartType: "histogram",
                  yAxis: yAxisColumn,
                  limit: 1000,
                ...
                console.log("Auto-generating histogram with config:", defaultConfig)
                handleGenerateChart(defaultConfig)
              }
            }
          }
  ...
  const validateConfig = (config) => {
    ...
    if (["histogram", "boxplot"].includes(config.chartType)) {
      if (!config.yAxis) {
        ...
        errors.push(`Column "${config.yAxis}" not found. Available columns: ${headerNames.join(", ")}`)
      }
    } else {
  ...
  const handleGenerateChart = async (config) => {
    ...
    if (validationErrors.length > 0) {
      ...
      clearError()
      // Set error after a brief delay to ensure it shows
      setTimeout(() => {
    ...
    try {
      console.log("DataDistributionChart: Generating chart with config:", config)
      ...
      result = await generateAggregateData(selectedFileId, requestConfig)
...
• Column names match exactly (case-sensitive)`)
        ...
        console.log("DataDistributionChart: Chart generated successfully:", chartResult)
      } else {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Data Distribution Analysis</h3>
      ...
      <div className="h-80 mb-4">
        {localLoading ? (
          ...
          <ChartRenderer chartData={chartData} chartType={currentConfig?.chartType || "bar"} title={chartData.title} />
        ) : (
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              ...
              <p className="text-lg font-semibold text-gray-900 dark:text-white">{distributionStats.count}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              <p className="text-xs text-gray-600 dark:text-gray-400">Mean</p>
              <p className="text-lg font-semibold text-gray-900 dark:text-white">{distributionStats.mean}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              <p className="text-xs text-gray-600 dark:text-gray-400">Median</p>
              <p className="text-lg font-semibold text-gray-900 dark:text-white">{distributionStats.median}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              <p className="text-xs text-gray-600 dark:text-gray-400">Std Dev</p>
              <p className="text-lg font-semibold text-gray-900 dark:text-white">{distributionStats.stdDev}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <ChartConfigModal
        ...
        allowedTypes={["histogram", "boxplot", "bar", "pie", "doughnut", "scatter"]}
      />
    </div>
  )
}
