...
const TrendAnalysisChart = ({ selectedFileId, className = "" }) => {
  // Using REMAINING ChartContext modules not used in enhanced data distribution
  const {
    loading, // SHARED: Global loading state
    error,
    chartData, // SHARED: Global chart data
    headers,
    fetchHeaders,
    generate3DChart, // NEW: 3D chart generation
    generateAggregateData,
    clearError,
    clearChartData, // SHARED: Clear chart data
  } = useChart()
        ...
        .then((result) => {
          if (result && result.length >= 2) {
            ...
            const textHeaders = result.filter(
              (h) => !["numeric", "number", "float", "integer", "decimal"].includes(h?.type || h?.dataType || "text"),
            )

            if (numericHeaders.length > 0) {
              const yAxisColumn = numericHeaders[0]?.name
              const xAxisColumn =
                textHeaders.length > 0 ? textHeaders[0]?.name : result.find((h) => h?.name !== yAxisColumn)?.name
  ...
  const calculateTrendMetrics = (data) => {
    ...
    const trendDirection =
      upwardTrends > downwardTrends ? "upward" : downwardTrends > upwardTrends ? "downward" : "stable"
  ...
  const handleGenerateChart = async (config) => {
    ...
    try {
      ...
      if (result && result.success && result.data && Array.isArray(result.data)) {
        ...
        const finalData = {
          labels: result.data.map((item) => item.label || item.name || "Unknown"),
          datasets: [
            {
        ...
        const chartResult = {
          ...
          xAxis: config.xAxis,
          yAxis: config.yAxis,
          title: config.chartTitle || `${config.yAxis} Trend Over ${config.xAxis}`,
        }
  ...
  const calculateTrendInsights = (data) => {
    ...
    if (percentChange > 10) {
      insights.push({
        type: "positive",
        message: `Strong upward trend: ${percentChange.toFixed(1)}% increase from start to end`,
      })
    } else if (percentChange > 5) {
      insights.push({
        type: "positive",
        message: `Moderate upward trend: ${percentChange.toFixed(1)}% increase from start to end`,
      })
    } else if (percentChange < -10) {
      insights.push({
        type: "negative",
        message: `Strong downward trend: ${Math.abs(percentChange).toFixed(1)}% decrease from start to end`,
      })
    } else if (percentChange < -5) {
      insights.push({
        type: "negative",
        message: `Moderate downward trend: ${Math.abs(percentChange).toFixed(1)}% decrease from start to end`,
      })
    } else {
      insights.push({
        type: "neutral",
        message: `Stable trend: ${Math.abs(percentChange).toFixed(1)}% change from start to end`,
      })
    }
    ...
    insights.push({
      type: "info",
      message: `Peak value: ${maxValue} at ${labels[maxIndex] || `point ${maxIndex + 1}`}`,
    })
    insights.push({
      type: "info",
      message: `Lowest value: ${minValue} at ${labels[minIndex] || `point ${minIndex + 1}`}`,
    })
  ...
  const handleExport = () => {
    ...
    try {
      const exportData = {
        ...
        trendData: localChartData
          ? localChartData.data.labels.map((label, index) => ({
              ...
              [localChartData.yAxis]: localChartData.data.datasets[0].data[index],
            }))
          : null,
      ...
      if (chart3DData) {
        csvContent.push("", "# 3D Chart Data", JSON.stringify(chart3DData, null, 2))
      }
      ...
      if (trendInsights.length > 0) {
        csvContent.push(
          ...
          "# Trend Insights",
          "Insight,Type",
          ...trendInsights.map((i, idx) => `"${idx + 1}. ${i.message}",${i.type}`),
        )
      }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Enhanced Trend Analysis</h3>
        <div className="flex space-x-2">
          ...
          {(localChartData || chart3DData) && (
            <>
              ...
              <button onClick={handleExport} className="p-2 text-gray-600 dark:text-gray-400">
            ...
            <div className="p-2 bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800 rounded-md">
              <div className="flex items-center justify-between">
                <p className="text-xs text-cyan-600 dark:text-cyan-400">
                  Global Chart Data: {globalChartInfo.dataSize} bytes | Keys: {globalChartInfo.dataKeys.join(", ")}
                </p>
                <button onClick={() => setShowStatusMessages(false)} className="text-cyan-400 hover:text-cyan-600">
                  <X className="h-3 w-3" />
                </button>
              </div>
            </div>
          ...
          <div className="p-2 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-md">
            <div className="flex items-center justify-between">
              <p className="text-xs text-purple-600 dark:text-purple-400">
                ...
                {chartDimension === "3d" ? "generate3DChart()" : "generateAggregateData()"}
              </p>
              <button onClick={() => setShowStatusMessages(false)} className="text-purple-400 hover:text-purple-600">
                <X className="h-3 w-3" />
              </button>
            </div>
          </div>
            ...
            <div className="p-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
              <div className="flex items-center justify-between">
                <p className="text-xs text-green-600 dark:text-green-400">
                  Headers: {headers.length} | 2D Chart: {localChartData ? "✓" : "✗"} | 3D Chart:{" "}
                  {chart3DData ? "✓" : "✗"}
                </p>
      ...
      {show3DChart && chart3DData && (
        <div className="mb-4 p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-md">
          <h4 className="text-sm font-medium text-purple-700 dark:text-purple-300 mb-2">3D Chart Data</h4>
          <div className="bg-purple-100 dark:bg-purple-800 p-3 rounded max-h-40 overflow-auto">
            <pre className="text-xs text-purple-700 dark:text-purple-300">{JSON.stringify(chart3DData, null, 2)}</pre>
          </div>
        </div>
      )}
          ...
          chartDimension === "3d" && chart3DData ? (
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <p className="text-center text-gray-600 dark:text-gray-400">
                ...
                <br />
                <span className="text-sm">Toggle 3D data view above to see raw data</span>
              </p>
            </div>
          ) : (
            localChartData && (
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg h-auto min-h-[80px] flex flex-col">
              <p className="text-xs text-gray-600 dark:text-gray-400 font-medium mb-2">Overall Change</p>
              <p
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg h-auto min-h-[80px] flex flex-col">
              <p className="text-xs text-gray-600 dark:text-gray-400 font-medium mb-2">Volatility</p>
              <p className="text-xl font-bold text-gray-900 dark:text-white">{trendMetrics.volatility}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg h-auto min-h-[80px] flex flex-col">
              <p className="text-xs text-gray-600 dark:text-gray-400 font-medium mb-2">Direction</p>
              <p
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg h-auto min-h-[80px] flex flex-col">
              <p className="text-xs text-gray-600 dark:text-gray-400 font-medium mb-2">Average</p>
              <p className="text-xl font-bold text-gray-900 dark:text-white">{trendMetrics.mean}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {trendInsights.length > 0 && (
        <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
          <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Trend Insights</h4>
      ...
      <ChartConfigModal
        ...
        headers={headers}
        title={`Configure ${chartDimension === "3d" ? "3D" : "2D"} Trend Analysis Chart`}
        allowedTypes={
          chartDimension === "3d" ? ["scatter3d", "surface3d", "mesh3d"] : ["line", "area", "bar", "scatter"]
        }
      />
    </div>
  )
