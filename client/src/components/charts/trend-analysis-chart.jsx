...
const TrendAnalysisChart = ({ selectedFileId, className = "" }) => {
  const { error, headers, fetchHeaders, generateChart, clearError } =
    useChart();
  ...
  useEffect(() => {
    if (selectedFileId) {
      ...
      fetchHeaders(selectedFileId)
        .then((result) => {
          if (result.length >= 2 && !chartData) {
            const defaultConfig = {
              chartType: "line",
              xAxis: result[0]?.name,
              yAxis: result[1]?.name,
              limit: 100,
              chartTitle: `${result[1]?.name} Trend Over ${result[0]?.name}`,
            };
  ...
  const handleGenerateChart = async (config) => {
    ...
    try {
      const result = await generateChart(selectedFileId, {
        xAxis: config.xAxis,
        yAxis: config.yAxis,
