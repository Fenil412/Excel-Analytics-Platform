...
import { Settings, RefreshCw, Download, TrendingUp, AlertTriangle, Activity } from "lucide-react"
import { useChart } from "../../contexts/ChartContext"
import ChartConfigModal from "./chart-config-modal"
import ChartRenderer from "./chart-renderer"

const TrendAnalysisChart = ({ selectedFileId, className = "" }) => {
  const { error, headers, fetchHeaders, generateAggregateData, clearError } = useChart()
          ...
          if (result && result.length >= 2) {
            ...
            if (numericHeaders.length > 0) {
              ...
              if (yAxisColumn && xAxisColumn) {
                ...
                console.log("Auto-generating trend chart with config:", defaultConfig)
  ...
  const validateConfig = (config) => {
    ...
    if (!config.xAxis) {
      ...
      errors.push(`X-axis column "${config.xAxis}" not found. Available columns: ${headerNames.join(", ")}`)
    }

    if (!config.yAxis) {
      errors.push("Y-axis column is required")
    } else if (!headerNames.includes(config.yAxis)) {
      errors.push(`Y-axis column "${config.yAxis}" not found. Available columns: ${headerNames.join(", ")}`)
    }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Advanced Trend Analysis</h3>
        <div className="flex space-x-2">
          {chartData && (
            <>
              <button onClick={handleRefresh} disabled={localLoading} className="p-2 text-gray-600 dark:text-gray-400">
          ...
          >
            <Settings className={`h-4 w-4 ${headersLoading ? "animate-spin" : ""}`} />
          </button>
        </div>
      </div>
      ...
      {selectedFileId && headersLoading && (
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
          <p className="text-sm text-blue-600 dark:text-blue-400">Loading column headers...</p>
        </div>
      )}
      ...
      {selectedFileId && !headersLoading && headers.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
          <p className="text-sm text-yellow-600 dark:text-yellow-400">
            No headers found. Please check if the file contains valid data or try refreshing.
          </p>
        </div>
      )}
      ...
      {selectedFileId && !headersLoading && headers.length > 0 && (
        <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
          <p className="text-sm text-green-600 dark:text-green-400">
            Found {headers.length} columns: {headers.map((h) => h?.name || h || "Unknown").join(", ")}
          </p>
        </div>
      )}
      ...
      <div className="h-80 mb-4">
        {localLoading ? (
          ...
          <ChartRenderer chartData={chartData} chartType={currentConfig?.chartType || "line"} title={chartData.title} />
        ) : (
      ...
      {trendMetrics && (
        <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mb-4">
          <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Trend Metrics</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              <p className="text-xs text-gray-600 dark:text-gray-400">Overall Change</p>
              <p
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              ...
              <p className="text-lg font-semibold text-gray-900 dark:text-white">{trendMetrics.volatility}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              ...
              <p className="text-lg font-semibold text-gray-900 dark:text-white">{trendMetrics.mean}</p>
            </div>
          </div>
        </div>
      )}
