...
exports.chartData = async (req, res) => {
    try {
        ...
        if (!headers || !Array.isArray(rows)) {
            return res
                .status(400)
                .json({ error: "Invalid dataset structure: headers or rows missing" });
        }
        ...
        const formattedData = DataProcessor.formatDataForChart(
            chartData,
            chartType
        );
...
exports.aggregateData = async (req, res) => {
    try {
        const { fileId } = req.params;
        const { groupBy, aggregateField, aggregateFunction } = req.body;
